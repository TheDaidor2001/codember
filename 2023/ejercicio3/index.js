function getSecurePassword(array) {
  const validPasswords = [];
  const invalidPasswords = [];

  array.forEach((pass) => {
    const divided = pass.split(" ");
    const firstItem = divided[0].split("-");
    const secondItem = divided[1].split(":");
    const password = divided[2];
    let contador = 0;

    for (let i = 0; i < password.length; i++) {
      if (password[i] === secondItem[0]) {
        contador++;
      }
    }

    if (contador >= firstItem[0] && contador <= firstItem[1]) {
